...
def filter_normalise_zillow_data(raw_data, data_type):
    ...
    for row in raw_data:
        ...
        if row_state != zillow_column_state and row_state in amfam_territory_states:
            ...
            filtered_data.append(normalise_zillow_row(row, data_type))
    return filtered_data


def normalise_zillow_row(row, data_type):
    """
    ...
    :param data_type: type of data row being normalised
    ...
    # add the data type for differentiating condos and single family residences
    normalised_row.insert(county_name_index + 1, data_type)
